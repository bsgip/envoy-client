...
DEVICE_INFORMATION_CREATE_TEMPLATE = {"include": {}}
...
class BaseModel(PydanticBaseModel):
    ...
    class XmlTemplate:
        ...
        utility server API
        - link: this would usually match what a utility server would expect to respond
        to a GET request with. On the client, this is a convenience function for
        inspection.
        - show: a convenience template for showing the relationships between
        objects (for example an `EndDevice` linked to `DER`, rather than through a
        `DERListLink`)
        """
    ...
    def xml_dict(self, *args, **kwargs) -> dict:
        ...
        This creates an additional (single-entry) dictionary corresponding to the class
        name of the object.
        Note: This should only be called on a top-level document object - children
        objects should not use this method. The `to_xml` function is a better option to
        generate valid XML that conforms to the 2030.5 spec.
    ...
    def to_xml(self, mode="create", pretty=False) -> str:
        ...
        Args:
            mode (str, optional): Template to use for creating XML document. Defaults
              to 'create'.
            pretty (bool, optional): Include whitespace in resulting XML that preserves
        ...
        return xmltodict.unparse(
            self.xml_dict(mode=mode), full_document=False, pretty=pretty
        )


class PydanticList(BaseModel):
    """Sub-classing of a pydantic `BaseModel` that contains some convenience methods
    relating to the generation of valid XML from a list.

    The `list_field` attribute should correspond to name of the object attribute for
    which a list is to be generated. The generated dictionary is of the following
    structure:
    ...
    This matches the structure that `xmltodict` uses to generate XML with the following
    structure:
    ```xml
    <ObjectList>
        <Object>
...
class PropagateAliasBaseModel(BaseModel):
    class Config:
        propagate_aliases = True
        allow_population_by_field_name = True

    def dict(self, *args, **kwargs):
        if self.Config.propagate_aliases:
...
class XMLNSPrefixedAttribute(PropagateAliasBaseModel):
    def dict(self, *args, **kwargs):
        resp_dict = super().dict(*args, **kwargs)
        resp_dict.update(self.Config.xmlns)
        return resp_dict

    class Config(PropagateAliasBaseModel.Config):
...
class PollRateType(PropagateAliasBaseModel):
...
class ReactivePower(ValueWithMultiplier):
    ...
    rms current, and sin(theta) where theta is the phase angle of current relative to
    voltage.
    """
...
class DeviceInformation(BaseModel):
    ...
    functions_implemented: Optional[FunctionsImplementedType] = Field(
        alias="functionsImplemented",
        description=(
            "Bitmap indicating the function sets used by the device as a client"
        ),
    )
    gps_location: Optional[GPSLocationType] = Field(
        alias="gpsLocation", description="GPS location of this device"
    )
    lfdi: str = Field(alias="lFDI", description="Long form device identifier")
    mf_date: int = Field(
        alias="mfDate", default=0, description="Date/time of manufacture"
    )
    mf_hw_ver: Optional[str] = Field(
        alias="mfHwVer", default="Unknown", description="Manufacturer hardware version"
    )
    mf_id: Optional[int] = Field(
        alias="mfID", default=0, description="The manufacturer’s IANA Enterprise Number"
    )
    ...
    mf_model: Optional[str] = Field(
        alias="mfModel", default="Unknown", description="Manufacturer’s model number"
    )
    ...
    primary_power: Optional[PowerSourceType] = Field(
        alias="primaryPower",
        default=PowerSourceType.none,
        description="Primary source of power",
    )
    secondary_power: Optional[PowerSourceType] = Field(
        alias="secondaryPower",
        default=PowerSourceType.none,
        description="Secondary source of power",
    )
    sw_act_time: Optional[int] = Field(
        alias="swActTime",
        default=0,
        description="Activation date/time of currently running software",
    )
    sw_ver: Optional[str] = Field(
        alias="swVer", default="NA", description="Currently running software version"
    )

    class XmlTemplate:
        create = {"exclude_unset": False, "by_alias": True}
        show = create
        list = create
...
class DERCapability(BaseModel):
    ...
    referred to as its nameplate ratings. Ratings are read-only values established by
    the DER manufacturer by design or manufactured configuration, for instance, the
    continuous delivered active power rating capability in watts (rtgMaxW), and are
    available by reading the DERCapability resource.
    """
    ...
    rtg_max_charge_rate_va: Optional[ValueWithMultiplier] = Field(
        alias="rtgMaxChargeRateVA"
    )
    rtg_max_charge_rate_w: Optional[ValueWithMultiplier] = Field(
        alias="rtgMaxChargeRateW"
    )
    rtg_max_discharge_rate_va: Optional[ValueWithMultiplier] = Field(
        alias="rtgMaxDischargeRateVA"
    )
    rtg_max_discharge_rate_w: Optional[ValueWithMultiplier] = Field(
        alias="rtgMaxDischargeRateW"
    )
    type_: DERType = Field(alias="type")

    class XmlTemplate:
        create = {"by_alias": True, "exclude_none": True}
...
class ConnectionPoint(BaseModel):
    ...
    class XmlTemplate:
        create = {"include": {"connection_point_id", "meter_id"}, "by_alias": True}
        show = create
        list = create
...
class EndDevice(BaseModel):
    ...
    device_information_link: Optional[Link] = Field(
        default=None, alias="DeviceInformationLink"
    )
    ...
    def calculate_sfdi(cls, v, values):
        ...
        if lfdi and not v:
            ...
            sod_checksum = (
                10 - sum([int(digit) for digit in str(sfdi_no_sod_checksum)]) % 10
            )
...
class PowerOfTenMultiplierType(int):
    ...
    def validate(cls, value):
        ...
        if value > 9 or value < -9:
            raise ValueError(
                "PowerOfTenMultiplierType out of range -9 to 9 (Nano to Giga)"
            )
        return value
